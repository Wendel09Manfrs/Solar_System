...
export class SceneManager {
  ...
  init() {
    ...
    const ambientLight = new AmbientLight(0x99999999)
  ...
  createNoises() {
    ...
    this.noises.camera = new OrthographicCamera(-1, 1, 1, -1, 0.1, 10)
    this.noises.camera.position.set(0, 0, 5)
    this.noises.scene.add(this.noises.camera)

    this.noises.plane = {}
    this.noises.plane.geometry = new PlaneGeometry(2, 2)
    this.noises.plane.material = new ShaderMaterial({
      vertexShader: noisesVertex,
      fragmentShader: noisesFragment,
      blending: NoBlending,
    })
    this.noises.plane.mesh = new Mesh(
      this.noises.plane.geometry,
      this.noises.plane.material,
    )
    this.noises.scene.add(this.noises.plane.mesh)

    this.noises.renderTarget = new WebGLRenderTarget(128, 128, {
  ...
  createComposition() {
    this.composition = {}
    this.composition.defaultRenderTarget = new WebGLRenderTarget(
      this.sizes.width * this.renderer.getPixelRatio(),
      this.sizes.height * this.renderer.getPixelRatio(),
      { generateMipmaps: false, logarithmicDepthBuffer: true },
    )

    this.composition.distortionRenderTarget = new WebGLRenderTarget(
      this.sizes.width * this.renderer.getPixelRatio(),
      this.sizes.height * this.renderer.getPixelRatio(),
      {
        generateMipmaps: false,
    ...
    this.composition.scene = new Scene()
    this.composition.camera = new OrthographicCamera(
      -1,
      1,
      1,
      -1,
    ...
    this.composition.plane.geometry = new PlaneGeometry(2, 2)
    this.composition.plane.material = new ShaderMaterial({
      vertexShader: compositionVertex,
      fragmentShader: compositionFragment,
      uniforms: {
        uTime: { value: 0 },
        uDefaultTexture: {
          value: this.composition.defaultRenderTarget.texture,
        },
        uDistortionTexture: {
          value: this.composition.distortionRenderTarget.texture,
        },
        uConvergencePosition: { value: new Vector2() },
      },
      depthTest: true,
      depthWrite: false,
      blending: AdditiveBlending,
    })
    this.composition.plane.mesh = new Mesh(
      this.composition.plane.geometry,
      this.composition.plane.material,
    )
