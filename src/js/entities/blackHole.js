...
export class BlackHole {
  constructor(size, label ,pos, posHoleWhite) {
    this.scene = sceneManager.scene;
    this.camera = sceneManager.camera;
    this.pos = pos;
    this.size = size
    this.label = label
    this.speedDisk = -3e-11
    this.posHoleWhite = posHoleWhite

    this.angleBlackHole = -MathUtils.degToRad(150.2);
  ...
  createGradient() {
    ...
    this.gradient.style.addColorStop(0, '#ffff00');
    this.gradient.style.addColorStop(0.1, '#ff9900'); 
    this.gradient.style.addColorStop(0.2, '#ff3300'); 
    this.gradient.style.addColorStop(0.4, '#990033'); 
    this.gradient.style.addColorStop(0.8, '#000000'); 
  ...
  createMesh(sceneManager) {
    this.geometry = new CylinderGeometry(0.1, 6, 0, 64, 8, true);
    ...
    const innerGeometry = new CylinderGeometry(0.1, 0.1, 0, 64);
    const innerMaterial = new MeshBasicMaterial({ color: 0x000000 });
  ...
  createDistortion() {
    ...
    const angleDeg = -MathUtils.radToDeg(angle1);

    this.distortion = {};
    this.distortion.scene = new Scene()

    this.distortion.hole = {}
    this.distortion.hole.geometry = new PlaneGeometry(this.size, this.size);
    this.distortion.hole.material = new ShaderMaterial({
    ...
    this.distortion.hole.mesh = new Mesh(this.distortion.hole.geometry, this.distortion.hole.material);
    ...
    this.distortion.disc.geometry = new CircleGeometry(79.0734);
    this.distortion.disc.material = new ShaderMaterial({
      side: DoubleSide,
    ...
    this.distortion.disc.mesh = new Mesh(this.distortion.disc.geometry, this.distortion.disc.material);
  ...
  applyGrav(grav,camera) {
    ...
    const attractionDirection = new Vector3().copy(this.pos).sub(camera.position).normalize();
    camera.position.add(attractionDirection.multiplyScalar(attractionForce));
  
    if (distanceHole < 0.5) {
      camera.position.copy(this.posHoleWhite).add(new Vector3(0.1, 0.1, 0.1));
    }
  }
  }
}
